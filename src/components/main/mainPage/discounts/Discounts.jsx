...
const Discounts = (props) => {
    ...
    function OneProduct(name, price, discountPrice, img, description, key, good) {
        return (
            ...
            <div className={s.container}>
                <Slider {...settings}>
                    {props.discountGoods.map((item, id) => {
                        return OneProduct(item.name, item.prevPrice, item.discountPrice, item.img, item.description(), item.name, item)
                    })}
                </Slider>
            </div>
        </Fragment>
